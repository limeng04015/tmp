...
"""
量化系统 V5.2 最终闭环版
修复：KeyError、缺失函数、特征对齐、降级崩溃
内置：双策略 + Walk Forward回测 + 无未来函数 + 稳定数据源
"""
...
def build_ml_dataset(code):
    ...
    for i in range(120, len(df)-PREDICT_DAY):
        feat = {col:tech_df[col].iloc[i] if col in tech_df.columns else 0 for col in FEATURE_COLS}
...
def predict_future(rf, xgb_model, scaler, feat):
    ...
    feat_vec = [feat.get(c,0) for c in FEATURE_COLS]
    ...
    for style, ind_dict in STYLE_INDUSTRY_POOL.items():
        for ind_name, codes in ind_dict.items():
            ...
            for code in codes:
                ...
                _,_,_,_ = train_rolling_models(code)
                pred = predict_future(None, None, None, latest_feat)
                records.append({
                    "风格大类": style,
                    "名称": f"标的{code}",
                    "代码": code,
...
def run_final_system():
    ...
    print("="*120)
    print(f"【量化系统 V5.2 闭环终版｜{now}｜Walk Forward回测】")
    print("="*120)
    print(f"市场情绪得分：{get_market_sentiment():.1f}/100")

    print("\n===== 策略一：均衡分散多风格组合 =====")
    all_df, attack, balance, defense, inds = scan_balanced()
    print(f"覆盖行业：{inds}")
    print("\n🔥 进攻组合")
